...
class _ProjectProgressWidgetState extends State<ProjectProgressWidget> {
  ...
  Widget _buildProjectCard(_ProjectData pd, BuildContext context, Color canvasBg) {
    ...
    final bool isExpanded = _expandedProjectIds.contains(pd.projectId);
    final bool isSequoia = themeNotifier.value == AppTheme.sequoia;
    ...
    return Card(
      ...
      child: Column(
        ...
        children: [
          // HEADER TITRE + FLÈCHE D’EXPANSION
          InkWell(
            ...
            child: Container(
              decoration: BoxDecoration(
                color: AppColors.glassHeader, // glassHeader pour l’en-tête du projet
                borderRadius: const BorderRadius.vertical(top: Radius.circular(16)),
              ),
          ...
          // SI EXPANDU, AFFICHER LA LISTE DES TÂCHES
          if (isExpanded && totalTasks > 0) ...[
            const Padding(
              padding: EdgeInsets.symmetric(horizontal: 16.0),
              child: Divider(color: Colors.transparent, height: 1, thickness: 1),
            ),
            Padding(
              padding: const EdgeInsets.symmetric(horizontal: 16.0),
              child: Column(
                children: _buildTaskList(tasks, 0, canvasBg, context),
              ),
            ),
  ...
  /// - si pas de sous-tâches et que le statut vaut "completed" ou "terminé"/"terminée", ou
  ...
  List<Widget> _buildTaskList(
      List<CustomTask> tasks, int indent, Color canvasBg, BuildContext context) {
    ...
    for (final task in tasks) {
      ...
      if (hasSubtasks) {
        ...
        widgets.add(
          Padding(
            padding: EdgeInsets.only(left: indent * 12.0, right: 0),
            child: Theme(
              ...
              child: ExpansionTile(
                ...
                childrenPadding: EdgeInsets.only(left: (indent + 1) * 12.0, right: 0),
                title: Row(
                  children: [
                    Icon(icon,
                        size: 20,
                ...
                children: _buildTaskList(task.subTasks, indent + 1, canvasBg, context),
              ),
            ),
          ),
        );
        ...
        widgets.add(
          Padding(
            padding: EdgeInsets.only(left: indent * 12.0, right: 0, bottom: 4.0),
            child: Container(
              decoration: BoxDecoration(
                color: canvasBg, // glassBackground pour chaque tâche
